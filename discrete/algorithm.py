...
class RLTrainer:
    ...
    def eval_video(self, global_step: Union[int, None] = None):
        ...
        score = 0
        while not done and step_counter < 200:
            self.env.render()
...
if __name__ == '__main__':
    ...
    model_shape = (4, 84, 84)
    repeats = 4
    env = make_env('CarRacing-v2', env_shape, repeats, continuous=False)
    summary_writer = SummaryWriter()
    agent = DDQNAgent(alpha=0.0001, gamma=0.99, cnn_channel=[128, 256, 512], input_dims=model_shape,
                      fc_1_dims=512, fc_2_dims=512, n_actions=5, eps=1.0, eps_min=0.1, eps_dec=1e-5,
                      mem_size=50000, batch_size=200, prioritized_replay=True, replace_target=1000,
                      chkpt_dir="models/ddqn", summary_writer=summary_writer)

    trainer = RLTrainer(agent, env, summary_writer, n_games=300, update_step=2, render_interval=50)
